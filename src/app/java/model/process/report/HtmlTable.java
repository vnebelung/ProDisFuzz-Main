...
public class HtmlTable {
    ...
    public HtmlTable(int rows, int columns) {
        ...
        for (int i = 0; i < rows; i++) {
            table.appendChild(new Element("tr", ReportProcess.NAMESPACE));
            for (int j = 0; j < columns; j++) {
                table.getChildElements("tr").get(i).appendChild(new Element(i == 0 ? "th" : "td",
                        ReportProcess.NAMESPACE));
            }
        }
    }
    ...
    public void setText(int row, int column, String value) {
        table.getChildElements("tr").get(row).getChildElements(row == 0 ? "th" : "td").get(column).appendChild(value);
    }
    ...
    public void setLink(int row, int column, String href, String text) {
        ...
        a.appendChild(text);
        table.getChildElements("tr").get(row).getChildElements(row == 0 ? "th" : "td").get(column).appendChild(a);
    }
    ...
    public void setAttribute(int row, int column, String name, String value) {
        table.getChildElements("tr").get(row).getChildElements(row == 0 ? "th" : "td").get(column).addAttribute(new
