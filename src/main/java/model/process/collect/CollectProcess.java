...
public class CollectProcess extends AbstractProcess {
    ...
    /**
     * Sets all detected files in the already defined folder. Note: The method does not notify observers about a change
     * of the files.
     */
     ...
     */
    public ProtocolFile[] getFiles() {
        return files.toArray(new ProtocolFile[files.size()]);
    }
     ...
     */
    public ProtocolFile[] getSelectedFiles() {
        List<ProtocolFile> selection = files.stream().filter(each -> selected.get(each.getName())).map(each -> each)
                .collect(Collectors.toList());
        return selection.toArray(new ProtocolFile[selection.size()]);
    }
