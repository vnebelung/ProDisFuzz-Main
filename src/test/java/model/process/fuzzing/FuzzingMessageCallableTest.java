...
public class FuzzingMessageCallableTest {
    ...
    public void testCall() throws Exception {
        ...
        FuzzingMessageCallable fuzzingMessageCallable = new FuzzingMessageCallable(injectedProtocolStructure,
                FuzzOptionsProcess.InjectionMethod.SIMULTANEOUS);
        ...
        Assert.assertEquals(b1[0], new Byte(answer1[0]));
        Assert.assertEquals(b1[0], new Byte(answer1[answer1.length / 2]));
        Assert.assertEquals(b1[0], new Byte(answer2[0]));
        Assert.assertEquals(b1[0], new Byte(answer2[answer2.length / 2]));
        int count = 0;
        ...
        byte[] answer3;
        for (int i = 0; i < 5; i++) {
            answer3 = fuzzingMessageCallable.call();
            // Tests whether the correct bytes are present
            Assert.assertEquals(b1[0], new Byte(answer3[0]));
            Assert.assertEquals((byte) (65 + i), answer3[1]);
            Assert.assertEquals(b1[0], new Byte(answer3[2]));
            Assert.assertEquals((byte) (65 + i), answer3[3]);
        }

        // Test SEPARATE
        fuzzingMessageCallable = new FuzzingMessageCallable(injectedProtocolStructure, FuzzOptionsProcess
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Assert.assertEquals(b1[0], new Byte(answer4[0]));
            Assert.assertEquals(b1[0], new Byte(answer4[2]));
        ...
        for (int i = 0; i < 5; i++) {
            ...
            count = count + (answer4[1] == answer4[3] ? -1 : 1);
        }
        ...
        // Tests whether the correct fixed bytes are present
        Assert.assertEquals(b1[0], new Byte(answer6[0]));
        Assert.assertEquals(b1[0], new Byte(answer6[2]));
        // Test whether the first variable block is the correct library line
